...
sample_func = (lambda sample_seq: sample_seq[2])
...
programmers = [Student(stu[0],stu[1],stu[2]) for stu in student_tups_list]
...
names_only = [stu.name for stu in programmers]
names_and_productivities = list(zip(names_only, prod_list))
...
# Define len_check
def len_check(filelines):
    return (line for line in filelines if len(line) > 40)
